...
export default function MyClassifieds() {
  ...
  const [activeCategory, setActiveCategory] = useState(tMyClassifieds('tabs.all'))
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        const response = await classifiedsService.getPaymentStatus(pollingPaymentId)
        console.log('Payment status:', response)

        if (response.status === 'succeeded') {
          console.log('Платеж успешен, обновляем список объявлений')
          ...
          setPollingPaymentId(null)
          setErrorMessage('Платеж не удался. Попробуйте снова.')
        }
      } catch (error) {
        console.error('Ошибка проверки статуса платежа:', error)
        setPollingPaymentId(null)
        setErrorMessage('Ошибка проверки платежа. Попробуйте снова.')
      }
  ...
  useEffect(() => {
    ...
    if (refresh === 'true') {
      console.log('Обновление списка объявлений после успешной оплаты', {
  ...
  const handleToggleActive = async (id: string, currentIsActive: boolean) => {
    if (process.env.NODE_ENV !== 'production') {
      console.log('Переключение активности:', { id, currentIsActive })
    }
    try {
      const updated = await classifiedsService.toggleClassifiedActive(id, !currentIsActive)
      if (process.env.NODE_ENV !== 'production') {
        console.log('Обновленное объявление:', updated)
      }
      setClassifieds(prev =>
        prev.map(c => {
          if (c.id === id) {
            return {
              ...
              convertedPrice: c.convertedPrice ?? updated.convertedPrice ?? c.price,
  ...
  const handlePromote = async (id: string) => {
    if (process.env.NODE_ENV !== 'production') {
      console.log('Поднятие объявления:', { id })
    }
    try {
      const response = await classifiedsService.promoteClassified(id)
      console.log('Ответ на поднятие:', response)

      // Проверить наличие URL для оплаты
      if (response?.hutkoPayment?.checkout_url) {
        console.log('Открытие страницы оплаты в новой вкладке:', response.hutkoPayment.checkout_url)
        try {
          window.open(response.hutkoPayment.checkout_url, '_blank')
          // Начать polling для проверки статуса платежа
          if (response.hutkoPayment.payment_id) {
            setPollingPaymentId(response.hutkoPayment.payment_id)
          } else {
            setErrorMessage('Не получен payment_id для отслеживания платежа.')
          }
        } catch (redirectError) {
          console.error('Ошибка при открытии страницы оплаты:', redirectError)
          setErrorMessage('Не удалось открыть страницу оплаты. Попробуйте снова.')
        }
        return
      }

      console.error('Не получен URL для оплаты:', response)
      setErrorMessage('Ошибка инициализации платежа. Попробуйте позже.')
    } catch (error) {
      console.error('Ошибка поднятия объявления:', error)
      setErrorMessage('Ошибка при инициализации платежа. Попробуйте снова.')
    }
  }
    ...
    <div className='min-h-screen flex flex-col'>
      {isLoading ? (
        ...
        <div className='flex-1 pt-14 pb-16 md:pt-[88px] 2-5xl:pt-40!'>
          ...
          <NavigationButtons activePage={tMyClassifieds('buttons.myClassifieds')} />
