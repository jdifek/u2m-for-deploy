...
export const PaymentPlanSection = () => {
  ...
  // Обрабатываем параметры редиректа от Monobank
  useEffect(() => {
    const invoiceId = searchParams.get("invoiceId");
    const status = searchParams.get("status");
    console.log("PaymentPlanSection: Search params:", searchParams.toString(), {
      invoiceId,
      status,
    });
  ...
  }, [searchParams, updateUser, fetchNotifications, tPlan]);
  ...
  const handleChoosePlan = async (planId: string) => {
    ...
    try {
      ...
      console.log("handleChoosePlan response:", res);
      if (res.monobankPayment.pageUrl) {
        window.location.href = res.monobankPayment.pageUrl;
      } else {
        throw new Error("No payment redirect URL received");
      }
    } catch (error) {
      console.error("Error purchasing plan:", error);
      const errorMessage = tPlan("paymentError");
